...
기준: 2026-08-18 · 테스트 **140 통과, 실패 0** · 빌드 **경고 0**
...
⚠ **전제가 바뀌었다.** 전에는 *"`VNSaveData`는 대사 위치를 저장하는데
`EpisodeSelectionStateData`는 메모리에만 있다 — 진행 블록만 채우면 된다"*로 봤다.
그건 `test13`의 그림이다. **`dev`에는 세이브 층 자체가 없다**(위 ⚠ 박스).

그래서 G4는 두 갈래로 갈린다:

| | 뜻 |
|---|---|
| **쉬워진 것** | 맞춰야 할 기존 세이브 모양이 없다. `ProgressionSave`가 자유롭게 설계된다 |
| **어려워진 것** | 호스트 쪽 세이브 기반이 **아예 없다.** 누가 언제 만드는지가 이 패키지 밖 결정이다 |

**이 패키지의 몫은 변하지 않는다** — 진행 블록의 *모양*과 콘텐츠가 바뀐 뒤의 로드 규칙까지다.
어려운 건 직렬화가 아니라 **어제 콘텐츠로 저장하고 오늘 콘텐츠로 여는 것**이고, 규칙은
[`architecture.md`](architecture.md) §2.7의 표 6줄이다. 그중 조용해도 되는 것은
"새로 생긴 스탯을 `Initial`로 채우는 것" 하나뿐이다.

다만 **파일 포맷을 지금 확정할 이유가 사라졌다.** 호스트가 무엇으로 굽을지 정해지기 전에
`ProgressionSave`의 필드만 굳혀 두는 편이 안전하다.
