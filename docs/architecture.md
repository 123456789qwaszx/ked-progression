...
| 런타임 `PresentationCore/Flow` | 토큰·계획·커서·인플라이트 — **훌륭하다** | 실패 처리(`default: return false` = 데드락) |
| VnTool `Vn.Authoring.Chapters` | 간선에 관문, 스탯 경계, 검증기 — **맞다** | 엔진/엑셀에 묶여 이식 불가 |
...
| **D3** | `Tokens` · `Flags` | `Flags` 버림(v9가 `Stat` 0/1로 통일), `Tokens` 보류 |
